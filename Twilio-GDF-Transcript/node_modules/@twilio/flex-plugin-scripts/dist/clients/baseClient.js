...
var BaseClient = /** @class */ (function () {
    ...
    BaseClient.regions = ['dev', 'stage'];
    /**
     * Returns the base URL
     */
    BaseClient.getBaseUrl = function (subDomain, version) {
        var regions = BaseClient.regions;
        var region = flex_dev_utils_1.env.getRegion();
        if (region && !regions.includes(region)) {
            throw new Error("Invalid region " + region + " was provided. Region must be one of " + regions.join(','));
        }
        var regionDomain = region && regions.includes(region) ? "." + region.toLowerCase() : '';
